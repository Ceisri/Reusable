...
var max_fireballs: int = 50
var max_arcane_bolts: int = 50
...
func get_pool_for_projectile(projectile_scene: PackedScene) -> Array:
	if projectile_scene == fireball:
		return fireball_pool
	elif projectile_scene == arcane_bolt:
		return arcane_bolt_pool
	return []

func shootProjectile(damage, projectile_scene, damage_type, can_pentrate_wall, does_slow_down, is_lingering,speed,life_time):
	...
	projectile_instance = get_projectile_from_pool(projectile_scene)
	...
	projectile_instance.can_pentrate_wall = can_pentrate_wall
	projectile_instance.damage_type = damage_type
	projectile_instance.speed = speed
	projectile_instance.does_slow_down = does_slow_down
	projectile_instance.is_lingering = is_lingering
	projectile_instance.life_time = life_time
	...
	projectile_instance.set_physics_process(true)
